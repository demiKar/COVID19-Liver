...
seur_obj_hep <- seur_obj[,seur_obj$Major_compartments == "Hepatocytes"]

umap_1 <- as.data.frame(umap_embeddings$`0`)

plot_df <- cbind.data.frame(umap_1, seur_obj_hep$Label); 
...
figureC <- pheatmap_fixed(mat = dt,cluster_rows=F,cluster_cols =T,
